...
outer_ring_directions_list = [Hex(-2, 2, 0),
                              Hex(2, -1, -1),
                              Hex(2, -2, 0),
                              Hex(1, 1, -2),
                              Hex(0, 2, -2),
...
def inner_ring(coordinate):
    ...
    for i in range(len(inner_ring_directions_list)):
        output.append(hex_add(coordinate, inner_ring_directions_list[i]))
    return output
...
def outer_ring(coordinate):
    ...
    for i in range(len(outer_ring_directions_list)):
        output.append(hex_add(coordinate, outer_ring_directions_list[i]))
    return output
...
def select_target_location(animal_robot, platform_robot1, platform_robot2):
    ...
    move_list.remove(platform_robot1, platform_robot2)
